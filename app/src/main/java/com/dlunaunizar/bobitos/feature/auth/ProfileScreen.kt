...
// Antelación de los recordatorios: fila con etiqueta + menú desplegable de opciones.
@Composable
private fun ReminderLeadTimeSelector(selected: ReminderLeadTime, onSelect: (ReminderLeadTime) -> Unit) {
    ...
    Row(
        ...
        Text(stringResource(R.string.profile_reminders_lead_label), modifier = Modifier.weight(1f))
        Box {
            TextButton(onClick = { menu = true }) { Text(stringResource(selected.labelRes())) }
            DropdownMenu(expanded = menu, onDismissRequest = { menu = false }) {
                ReminderLeadTime.entries.forEach { option ->
                    DropdownMenuItem(
...
private fun ReminderLeadTime.labelRes(): Int = when (this) {
    ReminderLeadTime.AT_TIME -> R.string.reminder_lead_at_time
    ReminderLeadTime.MIN_10 -> R.string.reminder_lead_10
    ReminderLeadTime.MIN_30 -> R.string.reminder_lead_30
    ReminderLeadTime.MIN_60 -> R.string.reminder_lead_60
    ReminderLeadTime.DAY_BEFORE -> R.string.reminder_lead_day_before
}
