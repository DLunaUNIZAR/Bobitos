...
class FirestoreCalendarRepository @Inject constructor(
    ...
    override fun events(spaceId: String, rangeStart: Instant, rangeEndExclusive: Instant): Flow<List<CalendarEvent>> = callbackFlow {
        ...
        val registration = collection(spaceId)
            ...
            .addSnapshotListener { snapshot, error ->
                when {
                    ...
                    snapshot != null -> {
                        realtimeMetrics.snapshotReceived("calendar:visible-range", snapshot.documentChanges.size, snapshot.metadata.isFromCache)
                        trySend(snapshot.documents.mapNotNull(DocumentSnapshot::toCalendarEvent)
                            .filter { it.overlaps(rangeStart, rangeEndExclusive) }.sortedBy { it.startAt })
                    }
                }
            }
        awaitClose { registration.remove(); realtimeMetrics.listenerStopped(metricId) }
    }

    override suspend fun createEvent(spaceId: String, input: EventInput) = operation {
        val user = verifiedUser(); val value = input.validated()
    ...
    override suspend fun updateEvent(spaceId: String, eventId: String, input: EventInput) = operation {
        val user = verifiedUser(); val value = input.validated(); val event = collection(spaceId).document(eventId)
        val memberRefs = value.participantIds.map { membership(spaceId, it) }
        firestore.runTransaction { tx ->
            if (!tx.get(event).exists()) throw CalendarRepositoryException(CalendarFailure.EventNotFound)
            val members = memberRefs.map(tx::get); requireParticipants(value.participantIds, members)
    ...
    override suspend fun deleteEvent(spaceId: String, eventId: String) = operation {
        verifiedUser(); val event = collection(spaceId).document(eventId)
        firestore.runTransaction { tx ->
            if (!tx.get(event).exists()) throw CalendarRepositoryException(CalendarFailure.EventNotFound)
            tx.delete(event)
        }.await(); Unit
    ...
    private fun membership(spaceId: String, userId: String) = firestore.collection(MEMBERSHIPS).document("${spaceId}_$userId")
    private fun requireParticipants(ids: List<String>, docs: List<DocumentSnapshot>) {
        if (docs.zip(ids).any { (doc, id) -> !doc.exists() || doc.getString(FIELD_USER_ID) != id || doc.getString(FIELD_STATUS) != ACTIVE })
            throw CalendarRepositoryException(CalendarFailure.InvalidParticipants)
    }
    private fun verifiedUser(): AuthUser {
        val user = authRepository.currentUser.value ?: throw CalendarRepositoryException(CalendarFailure.NotAuthenticated)
    ...
    private suspend fun <T> operation(block: suspend () -> T): T = try {
        syncRepository.requireWritable(); block()
    } catch (e: CalendarRepositoryException) { throw e
    } catch (e: WriteNotAllowedException) { throw CalendarRepositoryException(CalendarFailure.Network, e)
    } catch (e: Throwable) { syncRepository.reportWriteFailure(e); throw e.asCalendarException() }

    private companion object {
        const val SPACES="spaces"; const val EVENTS="events"; const val MEMBERSHIPS="memberships"
        const val FIELD_START_AT="startAt"; const val FIELD_USER_ID="userId"; const val FIELD_STATUS="status"; const val FIELD_DISPLAY_NAME="displayName"
        const val ACTIVE="ACTIVE"; const val MAX_VISIBLE_EVENTS=250L
    }
}

private fun EventInput.validated(): EventInput {
    val title = title.trim(); val description = description?.trim()?.takeIf(String::isNotEmpty)
    when {
        ...
        description != null && description.length > 1000 -> throw CalendarRepositoryException(CalendarFailure.DescriptionTooLong)
        startAt >= endAt -> throw CalendarRepositoryException(CalendarFailure.InvalidRange)
        runCatching { ZoneId.of(timeZone) }.isFailure -> throw CalendarRepositoryException(CalendarFailure.InvalidTimeZone)
        participantIds.size > 10 || participantIds.distinct().size != participantIds.size || participantIds.any(String::isBlank) -> throw CalendarRepositoryException(CalendarFailure.InvalidParticipants)
        allDay && (startDate == null || endDateExclusive == null || startDate >= endDateExclusive) -> throw CalendarRepositoryException(CalendarFailure.InvalidRange)
        !allDay && (startDate != null || endDateExclusive != null) -> throw CalendarRepositoryException(CalendarFailure.InvalidRange)
    }
...
private fun EventInput.data(user: AuthUser, names: List<String>) = common(names) + mapOf(
    "createdBy" to user.id, "createdByName" to user.displayName.ifBlank { user.email.substringBefore('@') }.take(60),
    "createdAt" to FieldValue.serverTimestamp(), "updatedBy" to user.id, "updatedAt" to FieldValue.serverTimestamp(),
)
private fun EventInput.updateData(user: AuthUser, names: List<String>) = common(names) + mapOf(
    "updatedBy" to user.id, "updatedAt" to FieldValue.serverTimestamp(),
)
...
private fun DocumentSnapshot.toCalendarEvent(): CalendarEvent? {
    ...
    return CalendarEvent(id, getString("title") ?: return null, getString("description"), getBoolean("allDay") ?: return null,
        getTimestamp("startAt")?.toDate()?.toInstant() ?: return null, getTimestamp("endAt")?.toDate()?.toInstant() ?: return null,
        getString("startDate")?.let(LocalDate::parse), getString("endDateExclusive")?.let(LocalDate::parse), getString("timeZone") ?: return null,
        runCatching { EventColor.valueOf(getString("color").orEmpty()) }.getOrNull() ?: return null,
        (get("participantIds") as? List<*>)?.filterIsInstance<String>().orEmpty(), (get("participantNames") as? List<*>)?.filterIsInstance<String>().orEmpty(),
...
private fun Throwable.asCalendarException() = CalendarRepositoryException(when ((this as? FirebaseFirestoreException)?.code) {
